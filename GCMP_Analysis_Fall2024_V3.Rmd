...
phylo_noNA <- subset_taxa(phylo_noKing,Phylum != "Unclassified", Order!= "Chloroplast" & Family != "Mitochondria" & Order !="Unclassified bacteroidia")
...
set.seed(111)
coral <- 0
coral <- prune_taxa(taxa_sums(glom_orb) > 0, glom_orb)
coral
...
coral_taxa = cbind(as(mod1_w_coral, "data.frame"), as(tax_table(glom_orb)[rownames(mod1_w_coral), ], "matrix"))
#coral_taxa
write.csv(coral_taxa,"Caribbean Orbicella Samples Neutral Taxonomy table.csv",row.names = TRUE)
...
write.csv(env_taxa,"Colombia environmental Samples Neutral Taxonomy table.csv",row.names = TRUE)
...
lb2 <- mod_w_coral$m
lb3 <- expression(paste("Caribbean Orbicella \n Coral Samples"))
...
ev3 <- expression(paste("Colombia environmental \n Coral Samples"))
...
## Corals 
bray_coral = phyloseq::distance(all_coral, method="bray")
uni_coral = phyloseq::distance(all_coral, method="unifrac")
wuni_coral = phyloseq::distance(all_coral, method="wUnifrac")

# Create data frame for downstream analysis
coral_df <- data.frame(sample_data(all_coral))

## Robust Corals 
bray_col = phyloseq::distance(colombia.rarefied, method="bray")
uni_col = phyloseq::distance(colombia.rarefied, method="unifrac")
wuni_col = phyloseq::distance(colombia.rarefied, method="wUnifrac")

# Create data frame for downstream analysis
col_df <- data.frame(sample_data(colombia.rarefied))
...
rt.pcoa = ordinate(colombia.rarefied, method="PCoA", distance=wuni_col)
# Set variables to zero for subsequent changes
#Ofra
pcoa<-0
pcoa <- plot_ordination(colombia.rarefied, rt.pcoa,  shape = "run_lane",color="family")+  geom_point(size=3) + theme_bw() + theme(plot.title = element_text(hjust = 0.5)) + stat_ellipse(level = 0.95,type = "norm",aes(group=run_lane))+ theme(legend.text=element_text(size=12)) + theme(axis.text=element_text(size=12, face = "bold"), axis.title=element_text(size=14,face="bold") + theme(text = element_text(size = 20,face = "bold"))) # title =expression(paste(italic(" Orbicella spp")," Bray-Curtis Distance PCoA across Depth")) scale_color_manual(values=c("#E31A1C","#1F78B4"))+
