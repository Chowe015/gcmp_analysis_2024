...
colombia5 <-subset_samples(phylo_coral, country == "Colombia" & expedition_number == "E5" & sample_type_EMP == "coral")
colombia5
...
colombia12 <-subset_samples(phylo_coral, country == "Colombia" & expedition_number == "E12" & sample_type_EMP == "coral")
colombia12
...
colombia <-subset_samples(phylo_coral, country == "Colombia" & sample_type_EMP == "coral" )
colombia
...
col_orb <-subset_samples(phylo_coral, country == "Colombia" & host_genus_id == "Orbicella")# country == "Colombia" & expedition_number == "E5")
col_orb
...
panama <-subset_samples(phylo_coral, country == "Panama" & sample_type_EMP == "coral")
panama
...
panama.w <-subset_samples(phylo_coral, country == "Panama" & expedition_number == "E11" & sample_type_EMP == "coral")
panama.w
...
panama.e <-subset_samples(phylo_coral, country == "Panama" & expedition_number == "E2" & sample_type_EMP == "coral")
panama.e
...
## Plot distribution
glom_col <- tax_glom(colombia.rarefied, taxrank = 'Family', NArm = TRUE)## 198 samples
glom_col5 <- tax_glom(colombia.rarefied5, taxrank = 'Family', NArm = TRUE)## 95 samples
glom_col12 <- tax_glom(colombia.rarefied12, taxrank = 'Family', NArm = TRUE)## 103 samples
glom_col_orb <- tax_glom(col.orb.rarefied, taxrank = 'Family', NArm = TRUE)## 56 samples
glom_ofav <- tax_glom(faveolata.rarefied, taxrank = 'Family', NArm = TRUE)## 26 Samples from Colombia 


glom_pan <- tax_glom(panama.rarefied, taxrank = 'Family', NArm = TRUE)##  227 samples
glom_panw <- tax_glom(panama.w.rarefied, taxrank = 'Family', NArm = TRUE)## 83 samples
glom_pane <- tax_glom(panama.e.rarefied, taxrank = 'Family', NArm = TRUE)##  144 samples
glom_pan_orb <- tax_glom(pan.orb.rarefied, taxrank = 'Family', NArm = TRUE)## 38 samples

glom_orb <- tax_glom(orb.rarefied, taxrank = 'Family', NArm = TRUE)## 94 samples from the Caribbean
glom_env <- tax_glom(env.rarefied, taxrank = 'Family', NArm = TRUE)## 36 samples from Colombia, Panama only has one environmental sample
...
###coral communities
set.seed(111)
coral <- prune_taxa(taxa_sums(glom_col5) >0, glom_col5)
coral
...
coral_taxa = cbind(as(mod1_w_coral, "data.frame"), as(tax_table(glom_col5)[rownames(mod1_w_coral), ], "matrix"))
#coral_taxa
write.csv(coral_taxa,"Coloimbia E5 Coral Samples Neutral Taxonomy table.csv",row.names = TRUE)


set.seed(111)
###seawater communities
#seawater <- subset_samples(physeq2, SampleType == "seawater")
seawater <- prune_taxa(taxa_sums(glom_col12) >0, glom_col12)
seawater
...
write.csv(env_taxa,"Colombia E12 Coral Samples Neutral Taxonomy table.csv",row.names = TRUE)
...
lb2 <- mod_w_coral$m
lb3 <- expression(paste("Colombia E5 \n Coral Samples"))
...
ev3 <- expression(paste("Colombia E12 \n Coral Samples"))
