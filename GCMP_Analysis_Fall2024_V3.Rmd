...
phylo_noKing <-subset_taxa(main_coral, kingdom != "d__Eukaryota" & Phylum != "Unclassified d__Bacteria" & Order != "Unclassified Proteobacteria" & Family != "Mitochondria")

# Remove features that are identified as Chloroplast and Mitochondria or unclassified bacteria
phylo_noNA <- subset_taxa(phylo_noKing, kingdom != "Unassigned" & Phylum != "Unclassified Unassigned" & Order!= "Chloroplast" & Order !="Unclassified bacteroidia" & Class != "Unclassified Proteobacteria")
...
phylo_noKing
phylo_noNA
phylo_coral # 729 samples with 59796 unique asv
...
## Caribbean dataset rarefication to 1000 per sample for downstream analysis Retained 997,000 (15.42%) features in 997 (92.92%) samples at the specifed sampling depth.
...
colombia5 <-subset_samples(phylo_coral, political_area == "Colombia" & expedition_number == "E5" & sample_type_EMP == "coral" & outgroup == "n")
colombia5
...
colombia12 <-subset_samples(phylo_coral, political_area == "Colombia" & expedition_number == "E12" & sample_type_EMP == "coral"& outgroup == "n")
colombia12
...
colombia <-subset_samples(phylo_coral, political_area == "Colombia" & sample_type_EMP == "coral" & outgroup == "n")
colombia
...
col_orb <-subset_samples(phylo_coral, political_area == "Colombia" & host_genus_id == "Orbicella")# political_area == "Colombia" & expedition_number == "E5")
col_orb
...
faveolata <-subset_samples(phylo_coral, political_area == "Colombia" & host_species == "faveolata")
faveolata
...
panama <-subset_samples(phylo_coral, political_area == "Panama" & sample_type_EMP == "coral" & outgroup == "n")
panama
...
panama.w <-subset_samples(phylo_coral, political_area == "Panama" & expedition_number == "E11" & sample_type_EMP == "coral" & outgroup == "n")
panama.w
...
panama.e <-subset_samples(phylo_coral, political_area == "Panama" & expedition_number == "E2" & sample_type_EMP == "coral" & outgroup == "n")
panama.e
...
pan_orb <-subset_samples(phylo_coral, political_area == "Panama" & host_genus_id == "Orbicella")
pan_orb
...
## Overall Caribbean
carib <-subset_samples(phylo_coral, ocean_area == "Caribbean" & sample_type_EMP == "coral" & outgroup == "n")
carib

# rarefy to even depth
carib.rarefied = rarefy_even_depth(carib, rngseed=111, sample.size=1000, replace=F, trimOTUs = TRUE)
carib.rarefied

#### Environmental Samples 
env_sample <- subset_samples(phylo_coral, env_sample == "yes" & political_area == "Colombia")
env_sample
...
# keep result reproductive
water.rarefied = rarefy_even_depth(water, rngseed=111, sample.size=1000, replace=F, trimOTUs = TRUE)
water.rarefied
...
# keep result reproductive
sed.rarefied = rarefy_even_depth(sediment, rngseed=111, sample.size=1000, replace=F, trimOTUs = TRUE)
sed.rarefied

## Hawaii 
#### Subsample Panama east E2
hawaii <-subset_samples(phylo_coral, expedition_number == "E15" & sample_type_EMP == "coral" & outgroup == "n")
hawaii

# rarefy to even depth
hawaii.rarefied = rarefy_even_depth(hawaii, rngseed=111, sample.size=1000, replace=F, trimOTUs = TRUE)
hawaii.rarefied


## Aglomerate Taxa to Family Level
glom_col <- tax_glom(colombia.rarefied, taxrank = 'Family', NArm = TRUE)## 139 samples
glom_col5 <- tax_glom(colombia.rarefied5, taxrank = 'Family', NArm = TRUE)## 59 samples
glom_col12 <- tax_glom(colombia.rarefied12, taxrank = 'Family', NArm = TRUE)## 80 samples
glom_col_orb <- tax_glom(col.orb.rarefied, taxrank = 'Family', NArm = TRUE)## 27 samples
glom_ofav <- tax_glom(faveolata.rarefied, taxrank = 'Family', NArm = TRUE)## 10 Samples from Colombia 
glom_carib <-tax_glom(carib.rarefied, taxrank = 'Family', NArm = TRUE) ## 231

glom_pan <- tax_glom(panama.rarefied, taxrank = 'Family', NArm = TRUE)##  172 samples
glom_panw <- tax_glom(panama.w.rarefied, taxrank = 'Family', NArm = TRUE)## 80 samples
glom_pane <- tax_glom(panama.e.rarefied, taxrank = 'Family', NArm = TRUE)##  92 samples
glom_pan_orb <- tax_glom(pan.orb.rarefied, taxrank = 'Family', NArm = TRUE)## 33 samples

glom_orb <- tax_glom(orb.rarefied, taxrank = 'Family',  NArm = TRUE)## 60 samples from the Caribbean
glom_env <- tax_glom(env.rarefied, taxrank = 'Family', NArm = TRUE)## 45 samples from Colombia, Panama only has one environmental sample
glom_wat <- tax_glom(water.rarefied, taxrank = 'Family', NArm = TRUE)## 30 samples from Colombia, Panama only has one environmental sample
glom_sed <- tax_glom(sed.rarefied, taxrank = 'Family', NArm = TRUE)## 14 samples from Colombia, Panama only has one environmental 
glom_haw <- tax_glom(hawaii.rarefied, taxrank = 'Family', NArm = TRUE) ## 52 Samples 
glom_coral <- tax_glom(coral.rarefied, taxrank = 'Family', NArm = TRUE) ## 360 Samples 
```
...
#coral_taxa
write.csv(coral_taxa,"All Coral Samples Neutral Taxonomy table.csv",row.names = TRUE)


set.seed(111)
###seawater communities
#seawater <- subset_samples(physeq2, SampleType == "seawater")
seawater <- prune_taxa(taxa_sums(glom_haw) > 0, glom_haw)
seawater
...
## Assign Taxonomy to table of significant microbes
env_taxa = cbind(as(mod1_w_seawater, "data.frame"), as(tax_table(glom_haw)[rownames(mod1_w_seawater), ], "matrix"))

write.csv(env_taxa,"Hawaii E15 Coral Samples Neutral Taxonomy table.csv",row.names = TRUE)
...
lb2 <- mod_w_coral$m
lb3 <- expression(paste("All Coral Samples"))
...
ev3 <- expression(paste("Hawaii E15 \n Coral Samples"))
...
## All Corals 
bray_coral = phyloseq::distance(coral.rarefied, method="bray")
uni_coral = phyloseq::distance(coral.rarefied, method="unifrac")
wuni_coral = phyloseq::distance(coral.rarefied, method="wUnifrac")

# Create data frame for downstream analysis
coral_df <- data.frame(sample_data(coral.rarefied))


## Caribbean Corals 
bray_carib = phyloseq::distance(carib.rarefied, method="bray")
uni_carib = phyloseq::distance(carib.rarefied, method="unifrac")
wuni_carib = phyloseq::distance(carib.rarefied, method="wUnifrac")

# Create data frame for downstream analysis
carib_df <- data.frame(sample_data(carib))

## Robust Corals 
bray_pan = phyloseq::distance(panama.rarefied, method="bray")
uni_pan = phyloseq::distance(panama.rarefied, method="unifrac")
wuni_pan = phyloseq::distance(panama.rarefied, method="wUnifrac")

# Create data frame for downstream analysis
pan_df <- data.frame(sample_data(panama.rarefied))
...
#glom_flavo <-tax_glom(flavo, taxrank = 'Order', NArm = TRUE)

flavo <- subset_taxa(phylo_coral, Family== "Flavobacteriaceae")
...
# Create data frame for downstream analysis
flavo_df <- data.frame(sample_data(flavo_prune))

## Subset taxa of Cyclobacteriaceae
cyclo <- subset_taxa(phylo_coral, Family=="Cyclobacteriaceae")
cyclo_prune = prune_samples(sample_sums(cyclo)>1, cyclo)

## Cyclobacteriaceae  
bray_cyclo = phyloseq::distance(cyclo_prune, method="bray")
uni_cyclo = phyloseq::distance(cyclo_prune, method="unifrac")
wuni_cyclo = phyloseq::distance(cyclo_prune, method="wUnifrac")

# Create data frame for downstream analysis
cyclo_df <- data.frame(sample_data(cyclo_prune))

## Subset taxa of Endozoicomonadaceae
endo <- subset_taxa(phylo_coral, Family=="Endozoicomonadaceae")
endo_prune = prune_samples(sample_sums(endo)>1, endo)

## Endozoicomonadaceae  
bray_endo = phyloseq::distance(endo_prune, method="bray")
uni_endo = phyloseq::distance(endo_prune, method="unifrac")
wuni_endo = phyloseq::distance(endo_prune, method="wUnifrac")

# Create data frame for downstream analysis
endo_df <- data.frame(sample_data(cyclo_prune))
```

# Ordination from Distance Matrix and PCoA Plots 
```{r}
##Calculate the PCoA on Complex Corals
rt.pcoa = ordinate(coral.rarefied, method="PCoA", distance=bray_coral)
# Set variables to zero for subsequent changes
# expedition_number
pcoa<-0
pcoa <- plot_ordination(coral.rarefied, rt.pcoa,  shape = "expedition_number",color="run_prefix")+  geom_point(size=3) + theme_bw() + theme(plot.title = element_text(hjust = 0.5)) + stat_ellipse(level = 0.95,type = "norm",aes(group=run_prefix))+ theme(legend.text=element_text(size=12)) + theme(axis.text=element_text(size=12, face = "bold"), axis.title=element_text(size=14,face="bold") + theme(text = element_text(size = 20,face = "bold"))) # title =expression(paste(italic(" Orbicella spp")," Bray-Curtis Distance PCoA across Depth")) scale_color_manual(values=c("#E31A1C","#1F78B4"))+
...
for (i in distance_methods){ 
  form <- as.formula(paste(i,"expedition_number+family+BiologicalMatter+run_prefix+depth", sep="~"))
  print(form)
...
bray_coral_pairwise <-pairwise.perm.manova(bray_coral, coral_df$expedition_number,nperm=1000,p.method = "BH") 
uni_coral_pairwise <-pairwise.perm.manova(uni_coral,coral_df$expedition_number,nperm=1000,p.method = "BH")
wuni_coral_pairwise <-pairwise.perm.manova(wuni_coral,coral_df$expedition_number,nperm=1000,p.method = "BH")
...
# run hclust command
hc <- hclust(wuni_endo, method = "ward.D2")

# Get Tree tip label names 
tip_label <- endo_dd$BiologicalMatter
