...
phylo_noKing
phylo_noNA
phylo_coral # 729 samples with 60304 unique asv
...
coral <-subset_samples(phylo_coral, sample_type_EMP == "coral" & outgroup == "no" & within_group == "yes") # all corals without out groups, environmental samples & subset expeditions (i.e, singapore).
coral

# rarefy to even depth
coral.rarefied = rarefy_even_depth(coral, rngseed=111, sample.size=2500, replace=F, trimOTUs = TRUE)
coral.rarefied


colombia5 <-subset_samples(phylo_coral, political_area == "Colombia" & expedition_number == "E5" & sample_type_EMP == "coral" & outgroup == "no")
colombia5

# rarefy to even depth
colombia.rarefied5 = rarefy_even_depth(colombia5, rngseed=111, sample.size=2500, replace=F, trimOTUs = TRUE)
colombia.rarefied5

#### Subsample Colombia Expedition E12
colombia12 <-subset_samples(phylo_coral, political_area == "Colombia" & expedition_number == "E12" & sample_type_EMP == "coral")
colombia12

# rarefy to even depth
colombia.rarefied12 = rarefy_even_depth(colombia12, rngseed=111, sample.size=2500, replace=F, trimOTUs = TRUE)
colombia.rarefied12

#### Subsample Corals from Colombia includes E5 & E12
colombia <-subset_samples(phylo_coral, political_area == "Colombia" & sample_type_EMP == "coral" )
colombia

# rarefy to even depth
colombia.rarefied = rarefy_even_depth(colombia, rngseed=111, sample.size=2500, replace=F, trimOTUs = TRUE)
colombia.rarefied
...
# rarefy to even depth
col.orb.rarefied = rarefy_even_depth(col_orb, rngseed=111, sample.size=2500, replace=F, trimOTUs = TRUE)
col.orb.rarefied
...
faveolata.rarefied = rarefy_even_depth(faveolata, rngseed=111, sample.size=2500, replace=F, trimOTUs = TRUE)
faveolata.rarefied


############# Subsample Panama political_area including E2 & E11 East & West################
panama <-subset_samples(phylo_coral, political_area == "Panama" & sample_type_EMP == "coral")
panama

# rarefy to even depth
panama.rarefied = rarefy_even_depth(panama, rngseed=111, sample.size=2500, replace=F, trimOTUs = TRUE)
panama.rarefied

#### Subsample Panama west E11
panama.w <-subset_samples(phylo_coral, political_area == "Panama" & expedition_number == "E11" & sample_type_EMP == "coral")
panama.w

# rarefy to even depth
panama.w.rarefied = rarefy_even_depth(panama.w, rngseed=111, sample.size=2500, replace=F, trimOTUs = TRUE)
panama.w.rarefied

#### Subsample Panama east E2
panama.e <-subset_samples(phylo_coral, political_area == "Panama" & expedition_number == "E2" & sample_type_EMP == "coral")
panama.e

# rarefy to even depth
panama.e.rarefied = rarefy_even_depth(panama.e, rngseed=111, sample.size=2500, replace=F, trimOTUs = TRUE)
panama.e.rarefied
...
# rarefy to even depth
pan.orb.rarefied = rarefy_even_depth(pan_orb, rngseed=111, sample.size=2500, replace=F, trimOTUs = TRUE)
pan.orb.rarefied
...
# rarefy to even depth
orb.rarefied = rarefy_even_depth(orbicella, rngseed=111, sample.size=2500, replace=F, trimOTUs = TRUE)
orb.rarefied

## Overall Caribbean
carib <-subset_samples(phylo_coral, ocean_area == "Caribbean" & sample_type_EMP == "coral")
carib

# rarefy to even depth
carib.rarefied = rarefy_even_depth(carib, rngseed=111, sample.size=2500, replace=F, trimOTUs = TRUE)
carib.rarefied
...
env.rarefied = rarefy_even_depth(env_sample, rngseed=111, sample.size=2500, replace=F, trimOTUs = TRUE)
env.rarefied
...
# keep result reproductive
water.rarefied = rarefy_even_depth(water, rngseed=111, sample.size=2500, replace=F, trimOTUs = TRUE)
water.rarefied
...
# keep result reproductive
sed.rarefied = rarefy_even_depth(sediment, rngseed=111, sample.size=2500, replace=F, trimOTUs = TRUE)
sed.rarefied

## Hawaii 
#### Subsample Panama east E2
hawaii <-subset_samples(phylo_coral, expedition_number == "E15" & sample_type_EMP == "coral")
hawaii

# rarefy to even depth
hawaii.rarefied = rarefy_even_depth(hawaii, rngseed=111, sample.size=2500, replace=F, trimOTUs = TRUE)
hawaii.rarefied
...
glom_coral <- tax_glom(coral.rarefied, taxrank = 'Family', NArm = TRUE) ## 52 Samples 
```
...
set.seed(111)
coral <- 0
coral <- prune_taxa(taxa_sums(glom_wat) > 0, glom_wat)
coral
...
coral_taxa = cbind(as(mod1_w_coral, "data.frame"), as(tax_table(glom_wat)[rownames(mod1_w_coral), ], "matrix"))
#coral_taxa
write.csv(coral_taxa,"Colombia E5 & E12 Water Samples Neutral Taxonomy table.csv",row.names = TRUE)


set.seed(111)
###seawater communities
#seawater <- subset_samples(physeq2, SampleType == "seawater")
seawater <- prune_taxa(taxa_sums(glom_sed) > 0, glom_sed)
seawater
...
## Assign Taxonomy to table of significant microbes
env_taxa = cbind(as(mod1_w_seawater, "data.frame"), as(tax_table(glom_sed)[rownames(mod1_w_seawater), ], "matrix"))

write.csv(env_taxa,"Colombia E5 &E12 Sediment Samples Neutral Taxonomy table.csv",row.names = TRUE)
...
lb2 <- mod_w_coral$m
lb3 <- expression(paste("Colombia Water \n Coral Samples"))
...
ev3 <- expression(paste("Colombia Sediment \n Coral Samples"))
...
```{r}
## Distance Matrices
#all_coral <- subset_samples(phylo_coral, sample_type_EMP == "coral")
...
rt.pcoa = ordinate(flavo_prune, method="PCoA", distance=wuni_flavo)
# Set variables to zero for subsequent changes
#Ofra
pcoa<-0
pcoa <- plot_ordination(flavo_prune, rt.pcoa,  shape = "BiologicalMatter",color="expedition_number")+  geom_point(size=3) + theme_bw() + theme(plot.title = element_text(hjust = 0.5)) + stat_ellipse(level = 0.95,type = "norm",aes(group=expedition_number))+ theme(legend.text=element_text(size=12)) + theme(axis.text=element_text(size=12, face = "bold"), axis.title=element_text(size=14,face="bold") + theme(text = element_text(size = 20,face = "bold"))) # title =expression(paste(italic(" Orbicella spp")," Bray-Curtis Distance PCoA across Depth")) scale_color_manual(values=c("#E31A1C","#1F78B4"))+
