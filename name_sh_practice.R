...
glom_coral <- tax_glom(coral.rarefied, taxrank = 'Family', NArm = TRUE)

# Join otu and taxonomy tables by id
paste(print("Joining OTU and taxonomy tables from Agglomerated phyloseq object..."))
phyloseq::otu_table(glom_coral)%>%
        as.data.frame()%>%
        rownames_to_column("id")%>%
        right_join(phyloseq::tax_table(glom_coral)%>%
        as.data.frame()%>%
...
write.csv(coral_biom, file ="GCMP24_rarefied_coral_table.csv")
