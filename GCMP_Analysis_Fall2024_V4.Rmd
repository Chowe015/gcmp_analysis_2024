...
coral <-subset_samples(phylo_coral, sample_type_EMP == "coral" & outgroup == "n") # all corals without out groups, environmental samples & subset expeditions (i.e, singapore).
coral
...
mucus <-subset_samples(coral, political_area == "Colombia" & BiologicalMatter == "Coral Mucus")
mucus
...
tissue <-subset_samples(coral, political_area == "Colombia" & BiologicalMatter == "Coral Tissue")
tissue
# rarefy to even depth
...
#### Subsample Panama east E12 E5
skeleton <-subset_samples(coral, political_area == "Colombia" & BiologicalMatter == "Coral Skeleton")
skeleton
# rarefy to even depth
...
sncm.LL <- function(m, sigma){
	R = freq - pbeta(d, N*m*p, N*m*(1-p), lower.tail=FALSE) #pbeta Density, distribution function, quantile function and random generation for the Beta distribution with parameters shape1 and shape2 (and optional non-centrality parameter ncp).
	R = dnorm(R, 0, sigma) # dnorm Density, distribution function, quantile function and random generation for the normal distribution with mean equal to mean and standard deviation equal to sd.
	-sum(log(R))
}
...
# expedition_number
pcoa<-0
pcoa <- plot_ordination(coral.rarefied, rt.pcoa,  shape = "expedition_number",color="sample_type")+  geom_point(size=3) + theme_bw() + theme(plot.title = element_text(hjust = 0.5)) + stat_ellipse(level = 0.95,type = "norm",aes(group=sample_type))+ theme(legend.text=element_text(size=12)) + theme(axis.text=element_text(size=12, face = "bold"), axis.title=element_text(size=14,face="bold") + theme(text = element_text(size = 20,face = "bold")))
...
for (i in distance_methods){ 
  form <- as.formula(paste(i,"expedition_number+family+sample_type+Huang_Roy_tree_name+site_name", sep="~"))
  print(form)
...
bray_carib_pairwise <-pairwise.perm.manova(bray_coral, coral_df$sample_type,nperm=1000,p.method = "BH") 
uni_carib_pairwise <-pairwise.perm.manova(uni_coral,coral_df$sample_type,nperm=1000,p.method = "BH")
wuni_carib_pairwise <-pairwise.perm.manova(wuni_coral,coral_df$sample_type,nperm=1000,p.method = "BH")
