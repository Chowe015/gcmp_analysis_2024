...
set.seed(111)
coral <- 0
coral <- prune_taxa(taxa_sums(glom_pan_ofav) > 0, glom_pan_ofav)
coral
...
coral_taxa = cbind(as(mod1_w_coral, "data.frame"), as(tax_table(glom_pan_ofav)[rownames(mod1_w_coral), ], "matrix"))
#coral_taxa
write.csv(coral_taxa,"Panama E2 Orbicella faveolata Coral Samples Neutral Taxonomy table.csv",row.names = TRUE)


set.seed(111)
###seawater communities
#seawater <- subset_samples(physeq2, SampleType == "seawater")
seawater <- prune_taxa(taxa_sums(glom_ofav) > 0, glom_ofav )
seawater
...
## Assign Taxonomy to table of significant microbes
env_taxa = cbind(as(mod1_w_seawater, "data.frame"), as(tax_table(glom_glom_col_orbofav)[rownames(mod1_w_seawater), ], "matrix"))

write.csv(env_taxa,"Colombia E5 & E12 Orbicella faveolata Coral Samples Neutral Taxonomy table.csv",row.names = TRUE)
...
lb2 <- mod_w_coral$m
lb3 <- expression(paste("Panama E2 \n Orbicella faveolata Coral Samples"))
...
ev3 <- expression(paste("Colombian E5 & E12 \n Orbicella faveolata Coral Samples"))
...
rt.pcoa = ordinate(carib.rarefied, method="PCoA", distance=bray_carib)
# Set variables to zero for subsequent changes
# expedition_number
pcoa<-0
pcoa <- plot_ordination(carib.rarefied, rt.pcoa,  shape = "expedition_number",color="BiologicalMatter")+  geom_point(size=3) + theme_bw() + theme(plot.title = element_text(hjust = 0.5)) + stat_ellipse(level = 0.95,type = "norm",aes(group=BiologicalMatter))+ theme(legend.text=element_text(size=12)) + theme(axis.text=element_text(size=12, face = "bold"), axis.title=element_text(size=14,face="bold") + theme(text = element_text(size = 20,face = "bold"))) # title =expression(paste(italic(" Orbicella spp")," Bray-Curtis Distance PCoA across Depth")) scale_color_manual(values=c("#E31A1C","#1F78B4"))+
...
distance_methods <-c("bray_carib","uni_carib","wuni_carib")
set.seed(129)

# This is what is called a for loop.        
for (i in distance_methods){ 
  form <- as.formula(paste(i,"expedition_number+family+BiologicalMatter", sep="~"))
  print(form)
 adonis2(form, data=carib_df)-> result
 print(result)
}
...
bray_carib_pairwise <-pairwise.perm.manova(bray_carib, carib_df$run_lane,nperm=1000,p.method = "BH") 
uni_carib_pairwise <-pairwise.perm.manova(uni_carib,carib_df$run_lane,nperm=1000,p.method = "BH")
wuni_carib_pairwise <-pairwise.perm.manova(wuni_carib,carib_df$run_lane,nperm=1000,p.method = "BH")
