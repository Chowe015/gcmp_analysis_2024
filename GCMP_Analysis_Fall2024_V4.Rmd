...
phylo_noKing <-subset_taxa(main_coral, kingdom != "d__Eukaryota" & Phylum != "Unclassified d__Bacteria" & Order != "Unclassified Proteobacteria" & Family != c("Mitochondria","Unknown_Family"))

# Remove features that are identified as Chloroplast and Mitochondria or unclassified bacteria
phylo_noNA <- subset_taxa(phylo_noKing, kingdom != "Unassigned" & Phylum != "Unclassified Unassigned" & Order!= "Chloroplast" & Order !="Unclassified bacteroidia" & Class != "Unclassified Proteobacteria" & Family != "Unclassified Alphaproteobacteria")
...
pan_ofav <-subset_samples(phylo_coral, political_area == "Panama" & host_species == "faveolata")
pan_ofav
...
orbicella <- subset_samples(phylo_coral, host_genus_id == "Orbicella" & expedition_number == "E12") 
...
mucus <-subset_samples(phylo_coral, political_area == "Colombia" & BiologicalMatter == "Coral Mucus")
mucus
...
tissue <-subset_samples(phylo_coral, political_area == "Colombia" & BiologicalMatter == "Coral Tissue")
tissue
# rarefy to even depth
...
#### Subsample Panama east E12 E5
skeleton <-subset_samples(phylo_coral, political_area == "Colombia" & BiologicalMatter == "Coral Skeleton")
skeleton
# rarefy to even depth
...
#glom_coral <- tax_glom(coral.rarefied, taxrank = 'Family', NArm = TRUE) ## 386 Samples 
...
## Calculate pvalue from Wilson CI intervals 

low <- pred.ci[,2]
high <- pred.ci[,3]
se <- (high-low)/(2*1.96)
zstat <- (freq/se)
padj <- (exp(-0.717*zstat-0.416*zstat^2))*N

##Results
	if(stats==TRUE){
		fitstats <- data.frame(m=numeric(), m.ci=numeric(), m.mle=numeric(), maxLL=numeric(), binoLL=numeric(), poisLL=numeric(), Rsqr=numeric(), Rsqr.bino=numeric(), Rsqr.pois=numeric(), RMSE=numeric(), RMSE.bino=numeric(), RMSE.pois=numeric(), AIC=numeric(), BIC=numeric(), AIC.bino=numeric(), BIC.bino=numeric(), AIC.pois=numeric(), BIC.pois=numeric(), N=numeric(), Samples=numeric(), Richness=numeric(), Detect=numeric(), se=numeric(), zstat=numeric(),padj=numeric())
	
			fitstats[1,] <- c(coef(m.fit), coef(m.fit)-m.ci[1], m.mle@coef['m'], m.mle@details$value, bino.mle@details$value, pois.mle@details$value, Rsqr, Rsqr.bino, Rsqr.pois, RMSE, RMSE.bino, RMSE.pois, aic.fit, bic.fit, aic.bino, bic.bino, aic.pois, bic.pois, N, nrow(spp), length(p), d, se,zstat,padj)
		return(fitstats)
	} else {
	A <- cbind(p, freq, freq.pred, pred.ci[,2:3], bino.pred, bino.pred.ci[,2:3], low, high, zstat, padj)
	A <- as.data.frame(A)
		colnames(A) <- c('relative abundance (p)', 'frequency', 'freq.predidction', 'pred.lwr', 'pred.upr','z-statistic', 'padj','low_test','high_test')
		if(is.null(taxon)){
			B <- A[order(A[,1]),]
		} else {
...
set.seed(111)
coral <- 0
coral <- prune_taxa(taxa_sums(glom_col12) > 0, glom_col12)
coral
...
coral_taxa = cbind(as(mod1_w_coral, "data.frame"), as(tax_table(glom_pane)[rownames(mod1_w_coral), ], "matrix"))
#coral_taxa
write.csv(coral_taxa,"Panama E12 Coral Samples Neutral Taxonomy table.csv",row.names = TRUE)


set.seed(111)
###seawater communities
#seawater <- subset_samples(physeq2, SampleType == "seawater")
seawater <- prune_taxa(taxa_sums(glom_col5) > 0, glom_col5 )
seawater
...
## Assign Taxonomy to table of significant microbes
env_taxa = cbind(as(mod1_w_seawater, "data.frame"), as(tax_table(glom_col5)[rownames(mod1_w_seawater), ], "matrix"))

write.csv(env_taxa,"Colombia E5 Samples Neutral Taxonomy table.csv",row.names = TRUE)
...
lb2 <- mod_w_coral$m
lb3 <- expression(paste("Colombia E5 Coral Samples"))
...
ev3 <- expression(paste("Colombian Environmental Samples"))
...
# Create data frame for downstream analysis
carib_df <- data.frame(sample_data(carib.rarefied))
...
## Subset taxa of Cyanobiaceae
cyclo <- subset_taxa(coral.rarefied, Family=="Cyanobiaceae", NArm = TRUE)
...
endo <- subset_taxa(coral.rarefied, Family=="Endozoicomonadaceae")
...
rt.pcoa = ordinate(flavo_prune, method="PCoA", distance=uni_flavo)
# Set variables to zero for subsequent changes
# expedition_number
pcoa<-0
pcoa <- plot_ordination(flavo_prune, rt.pcoa,  shape = "sample_type",color="run_prefix")+  geom_point(size=3) + theme_bw() + theme(plot.title = element_text(hjust = 0.5)) + stat_ellipse(level = 0.95,type = "norm",aes(group=sequence_run))+ theme(legend.text=element_text(size=12)) + theme(axis.text=element_text(size=12, face = "bold"), axis.title=element_text(size=14,face="bold") + theme(text = element_text(size = 20,face = "bold"))) # title =expression(paste(italic(" Orbicella spp")," Bray-Curtis Distance PCoA across Depth")) scale_color_manual(values=c("#E31A1C","#1F78B4"))+
...
distance_methods <-c("bray_flavo","uni_flavo","wuni_flavo")
set.seed(129)

# This is what is called a for loop.        
for (i in distance_methods){ 
  form <- as.formula(paste(i,"expedition_number+family+sample_type", sep="~"))
  print(form)
 adonis2(form, data=flavo_df)-> result
 print(result)
}
...
bray_carib_pairwise <-pairwise.perm.manova(bray_carib, carib_df$expedition_number,nperm=1000,p.method = "BH") 
uni_carib_pairwise <-pairwise.perm.manova(uni_carib,carib_df$expedition_number,nperm=1000,p.method = "BH")
wuni_carib_pairwise <-pairwise.perm.manova(wuni_carib,carib_df$expedition_number,nperm=1000,p.method = "BH")
