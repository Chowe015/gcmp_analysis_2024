...
asv <- qza_to_phyloseq(features = "./Australian_expeditions/input/nomito_table.qza")

#### Import Metadata read.table
metadata <- read.table(file = "./Australian_expeditions/input/aus_miseq_mapping_2024.csv",header=T,comment.char="", row.names=1, sep=",")

### Import Tree file from biom output tree.nwk
tree <- read_tree("./Australian_expeditions/input/tree.nwk")

coral_tree <-read_tree("./Australian_expeditions/input/huang_roy_tree.newick")

### Import taxonomy from biom output as .tsv format using read.table
taxonomy <- read.table(file = "./Australian_expeditions/input/taxonomy.tsv", sep = "\t", header = T ,row.names = 1, )
...
estimate_pd(coral.rarefied) %>%
  as.data.frame() %>%
  mutate('faithpd_avg' = mean(PD),
            'branch_length_avg' = mean(SR),
            'Faith_sd' = sd(PD), 
            'branch_sd' = sd(SR)) -> microbial_faith_pd
...
write.table(microbial_faith_pd, file ="microbial_faith_pd_table.csv", sep = ",",row.names =TRUE, col.names = TRUE)

write.csv(coral_faithpd_reorded, file ="coralHost_faith_pd_table.csv" ,row.names = TRUE)

#coral_table <- read.table(file = "../input/coral_expedition_matrix.csv",header=T,comment.char="", row.names=1, sep=",")
#coral_matrix <-as.matrix(coral_table)
