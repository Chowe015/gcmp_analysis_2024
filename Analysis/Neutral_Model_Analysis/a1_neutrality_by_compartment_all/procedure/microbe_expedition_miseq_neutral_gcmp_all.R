...
sink_name = paste0("GCMP_","Phyloseq_expedition_Analysis_log.txt")
...
subject = subset_samples(phylo, outgroup=="n" & subset_col=="yes")
subject
...
for (i in expedition){
# Subset only corals from database
subject <-subset_samples(glom, expedition_number == print(paste(i)))
subject
...
lb3 <- paste0(sample_name," Neutral Neutral Model")
...
neutral_file_name <- paste0("Miseq_",i,"_neutral_model_Plot.pdf")
...
exp_table_name <- paste0("Miseq_",i,"_neutral_model_statistical_results.txt")
write.table(neutral_results, file =exp_table_name,row.names = FALSE, col.names = TRUE)
}