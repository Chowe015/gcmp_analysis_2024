...
knitr::opts_knit$set(root.dir ='/Users/Colin Howe/Desktop/PSU Doctoral Program/GCMP_Working/Global GCMP Neutral Analysis 2024/full_gcmp_input')
...
# Subset only corals from database
subject <-subset_samples(phylo, outgroup == "n" & tissue_compartment == "S")
subject
...
#coral_taxa
write.csv(nonneutral_tax,"Merged Skeleton Neutral Taxonomy table.csv",row.names = TRUE)
...
```{r}
#plot neuModel
p_neuM_coral=ggplot(tissue_table, aes(x=log10(p), y=freq, color = model))+
...
## I need to create a metadata file for these samples using the neutral_fam_df as the new metadata. The sampleID can be the ASV names and 


# Select and filter significant taxa and important categories 
neutral_fam_df <- select(filter(nonneutral_tax, padj <=0.05),id,Kingdom,Phylum,Class,Order,Family,model,padj)

inner_join(nonneutral_tax,glom_otu_table, by = "id") -> neutral_table_tax
...
neutral_metadata <- sample_data(neutral_fam_df)

# create matrix format for OTU and taxonomy table
OTU2 <- otu_table(data.matrix(neutral_table, rownames.force = NA), taxa_are_rows = TRUE)
...
mucus_table <- read.table(file ="gcmp_qiime_input/Mucus_nuetral_model.csv",header=T,comment.char="", row.names=1, sep=",")
skeleton_table <- read.table(file ="gcmp_qiime_input/Skeleton_nuetral_model.csv",header=T,comment.char="", row.names=1, sep=",")
