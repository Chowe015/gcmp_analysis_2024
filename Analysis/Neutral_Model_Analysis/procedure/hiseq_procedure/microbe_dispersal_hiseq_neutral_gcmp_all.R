...
subject <-subset_samples(phylo_noking, outgroup == "n")
subject
...
for (i in group_a){
  for (g in group_b){
    # Subset only corals from database
    spp_subset <-subset_samples(glom, tissue_compartment == i)
    pool_subset <-subset_samples(glom, tissue_compartment == g)
    ...
    print(length(which(neutral_results_padj$model == "above")))
    print(length(which(neutral_results_padj$model == "below")))
    print(length(which(neutral_results_padj$model == "neutral")))
    ...
    psuedo_spp_name <- paste0(spp_name,"_subset_",pool_name,"_abundance_neutral_table.tsv")
    ...
    psuedo_pool_name <- paste0(pool_name,"_subset_",spp_name,"_abundance_neutral_table.tsv")
    ...
    lb1 <- paste0("R_squared",":",r)
    lb2 <- paste0("migration_rate (m):",m_var)
    lb3 <- paste0(paste0(spp_name,"&",pool_name," Disperal Neutral Model"))
    ...
    ncm_name <- paste0("Hiseq",g,"_",i,"_neutral_model_Plot.pdf")
