...
sink_name = paste0("Step2_",biosample,"_","Neutral_model_results_log.txt")
...
# import mapping file
#glom_mapping <-read.table("./tissue/T_glom_metadata.tsv", sep = "\t",header = TRUE,check.name=FALSE,row.names=1)
...
for (b in neutral){
 ...
 colnames(neutral_table)[1] <- "id" 
  ...
  glom_mapping %>% mutate(psuedo_sample_name = paste(colnames(neutral_table)[2:l]),
                          ...
                          neutrality = paste(b)) %>% 
    select(sample_name_first_demultiplexing, psuedo_sample_name, host_scientific_name,expedition_number,political_area,ocean_area,compartment_neutrality,compartment,neutrality) %>% 
...
glom_mapping %>% mutate(psuedo_sample_name = paste(colnames(neutral_table_test)[2:la]),
  ...
  select(psuedo_sample_name,sample_name_first_demultiplexing, host_scientific_name,expedition_number,political_area,ocean_area,compartment_neutrality,compartment,neutrality) %>% 
