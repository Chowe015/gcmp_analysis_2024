...
for (i in distance_methods){ 
  ...
  print(result)
anova_name = paste(biosample,i,"permanova_results.tsv")
write.table(result, file = anova_name, sep="\t",row.names = FALSE)
  }
...
neutral_uni_name <- paste0(biosample,"_neutrality_Unweighted_Unifrac.pdf")
ggsave(pcoa2, filename=neutral_uni_name, width = 15, height = 15, dpi= 600)
