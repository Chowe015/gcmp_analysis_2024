...
#glom_table <-read.table("./skeleton/S_glom_table.tsv", sep = "\t",header = TRUE,row.names=1,check.name=FALSE)
#glom_tax <-read.table("./skeleton/S_glom_taxonomy.tsv", sep = "\t",header = TRUE,row.names=1,check.name=FALSE)
#glom_mapping <-read.table("./skeleton/S_glom_metadata.tsv", sep = "\t",header = TRUE,row.names=1,check.name=FALSE)

print(paste("Imported GCMP OTU Table"))
## import table 
glom_table <- read.table(glom_table_path, sep = "\t",header = TRUE,row.names=1,check.name=FALSE)
#import taxonomy
glom_tax <- read.table(taxonomy_path, sep = "\t",header = TRUE,row.names=1,check.name=FALSE)

# import mapping file
glom_mapping <-read.table(metadata_path, sep = "\t",header = TRUE,row.names=1,check.name=FALSE)
...
main_df <-sample_data(main_phylo)
...
expedition <- c("E1","E2","E3","E5","E7","E10","E11","E12","E15")

for (i in expedition){
# Subset only corals from database
subject <-subset_samples(main_phylo, expedition_number == print(paste(i)))
subject

mapping <- sample_data((subject))
sample_name = mapping$sampling_expedition[2]
compart_name = mapping$tissue_compartment[2]
###coral communities
set.seed(111)
...
lb3 <- paste0(biosample," & ",i," Neutral Model")
lb4 <-paste0(sample_name," Log10 (Mean Relative abundance)")
lb5 <- paste0(sample_name," Occurance frequency")
...
p_neuM_coral1 = p_neuM_coral + annotate("text", x = -3.3, y=0.9, size=6, label=lb1, 
                                        fontface="bold", color="black",
                                        parse=TRUE) +
  annotate("text", x = -3.3, y=0.8, size=6, label=lb2, 
           fontface="bold", color="black",
           parse=TRUE)
...
neutral_file_name <- paste0("./expedition/",biosample,"/miseq_",biosample,"_",i,"_neutral_model_plot.pdf")
ggsave(p_neuM_coral1, filename=neutral_file_name)

neutral_mod_w %>% as.data.frame() -> neutral_results
# Print output files
print(paste("Writing Combined Metadata File"))
exp_table_name <- paste0("./expedition/",biosample,"/miseq_",biosample,"_",i,"_neutral_model_statistical_results.tsv")
write.table(neutral_results, file =exp_table_name,sep="\t",row.names = FALSE, col.names = TRUE)
}
print("finished!")