...
sink_name = paste0("step_13_GCMP_Miseq_Dispersal_envo_compartments_log.txt")
...
#asv <-qza_to_phyloseq("./miseq_complete_nomito_table.qza")
#taxonomy <-read.table("./taxonomy.tsv", sep = "\t", header = T ,row.names = 1)
#metadata <-read.table("./gcmp_complete_mapping25.txt",header=T,comment.char="", row.names=1, sep="\t")
...
# Initial Subset for agglomeration and remove outgroup or doubled samples
subject <- subset_samples(glom, outgroup=="n" & filter_col=="no" & country=="Colombia")
subject

env <- subset_samples(glom,BiologicalMatter==c("water","sediment") & country=="Colombia")
env
...
psuedo_spp_name <- paste0("./dispersal/environment/","miseq_",spp_id,"_subset_",pool_id,"_abundance_neutral_table.tsv")
  (write.table(nonneutral_tax, file =psuedo_spp_name,sep="\t", row.names = FALSE))

# Print output files
spp_sample_name <- paste0("./dispersal/environment/","miseq_",spp_id,"_",pool_id,"_neutral_model_statistical_results.txt")
...
length(which(pool1_w_coral1$model == "above"))
length(which(pool1_w_coral1$model == "below"))
length(which(pool1_w_coral1$model == "neutral"))
...
## Results table
psuedo_spp_name2 <- paste0("./dispersal/environment/","miseq_",i,"_subset_",g,"_abundance_neutral_table.tsv")
write.table(nonneutral_tax2, file =psuedo_spp_name2,sep="\t", row.names = FALSE,col.names = TRUE)
# Print output files
psuedo_pool_name <- paste0("./dispersal/environment/","miseq_",g,"_subset_",i,"_neutral_model_statistical_results.txt")
...
ncm_name <- paste0("./dispersal/environment/","miseq_",g,"_",i,"_neutral_model_plot.pdf")
